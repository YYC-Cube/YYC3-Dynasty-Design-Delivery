...
| 字段         | 必填 | 说明                 | 示例                                      |
| ------------ | ---- | -------------------- | ----------------------------------------- |
| file         | 必填 | 文件名（包含扩展名） | `useI18n.ts`                              |
| description  | 必填 | 一句话概括文件用途   | `国际化 Hook · 支持中文/English 动态切换` |
| author       | 必填 | 作者名称             | `YanYuCloudCube Team`                     |
| version      | 必填 | 语义化版本号         | `v1.0.0`                                  |
| created      | 必填 | 创建日期 YYYY-MM-DD  | `2026-03-05`                              |
| updated      | 必填 | 更新日期 YYYY-MM-DD  | `2026-03-05`                              |
| status       | 必填 | 文件状态（同 1.5）   | `active`                                  |
| tags         | 必填 | 标签列表 2-4 个      | `[hook],[i18n],[locale]`                  |
| copyright    | 可选 | 版权信息             | `YanYuCloudCube Team`                     |
| license      | 可选 | 许可证               | `MIT`                                     |
| brief        | 可选 | 简要说明             | `提供国际化功能`                          |
| details      | 可选 | 详细说明（多行）     | `支持中英文动态切换`                      |
| dependencies | 可选 | 依赖列表             | `React, Context API`                      |
| exports      | 可选 | 导出内容             | `useI18n, I18nProvider`                   |
| notes        | 可选 | 注意事项             | `需要在 App 根组件包裹`                   |
...
if [ $? -ne 0 ]; then
  echo "[失败] Header check failed. Please add proper headers to your files."
  exit 1
fi

echo "[通过] Header check passed."
exit 0
```
